...
export type ClipboardWriteFn = (text: string, signal: AbortSignal) => Promise<void>;
...
if (process.platform === "darwin") {
  ...
  candidates.push(["termux-clipboard-set", []]);
} else {
...
function tryWrite(command, args) {
...
for (const [command, args] of candidates) {
  if (await tryWrite(command, args)) {
    copied = true;
    break;
  }
}
...
if (!copied) process.exitCode = ${CLIPBOARD_HELPER_COPY_FAILED_EXIT_CODE};
`;
...
const candidates = [];
if (process.platform === "darwin") {
  candidates.push(["pbpaste", []]);
} else if (process.platform === "win32") {
  candidates.push(["powershell", ["-NoProfile", "-Command", "Get-Clipboard -Raw"]]);
} else if (process.env.TERMUX_VERSION) {
  candidates.push(["termux-clipboard-get", []]);
} else {
