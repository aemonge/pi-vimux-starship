...
import {
  ...
  type PromptRailColorize,
  type PromptRailSnapshot,
  PromptRailStore,
  renderPromptRails,
} from './prompt-rail.js';
import {
...
type ModalEditorOptions = {
  ...
  // Optional text transform applied to the label after its foreground color.
...
export class ModalEditor extends CustomEditor {
  private readonly appKeybindings: CustomEditorConstructorArgs[2];
  ...
  private borderOverride: ((text: string) => string) | null = null;
  ...
  // Optional label transform; null keeps foreground-only mode styling.
  ...
  private resolveSurfaceColor(
    ...
    if (policy === 'mode' || (policy === 'thinking' && this.borderBaseIsNeutral)) {
  ...
  // The border trap recolors the border getter, tracks host assignments, and
  // provides the temporary inactive override used during Neovim handoff. It is
  // installed for every configured editor surface; host-synced modes still
  // resolve to the byte-identical host color when no override is active.
  private installModeBorderColorizer(): void {
    ...
    const resolvedBorderColor = (text: string) =>
      this.borderOverride?.(text) ??
      this.resolveSurfaceColor(borderColorizers, this.borderSync, text, (s) => s);
  ...
  private getOpenLineRepeatReplayKeys(
    ...
    return Array.from({ length: overrideCount }, (_, index) =>
      index === 0 ? command.countOverrideKeys : ['j', ...command.countOverrideKeys],
    ).flat();
  }
  ...
  private getRepeatReplayKeys(
    ...
    if (this.isTextInsertRepeatCommand(command)) {
      ...
      const hasFinalEscape = finalKey !== undefined && isEscapeLikeInput(finalKey);
      const insertKeys = hasFinalEscape ? continuation.slice(0, -1) : continuation;
      return [
        startKey,
  ...
  private dispatchThroughSubmit(commandLine: string): void {
    ...
    if (this.exCommandSettings.copyInputToClipboard && dispatchState.snapshot.text) {
  ...
  private applyVisualOperator(operator: 'd' | 'y' | 'c', linewise: boolean): void {
    this.takeTotalCount(1);
  ...
  private swapVisualEnds(): void {
    ...
    const cursor = this.getCursor();
    this.visualAnchor = { line: cursor.line, col: cursor.col };
    this.moveCursorToAbsoluteIndex(this.getAbsoluteIndex(anchor.line, anchor.col));
  }
    ...
    if (this.prefixCount.length > 0) {
      ...
      if (supportsCountedNav) {
        ...
        if ((data === 'h' || data === 'l') && isVisualMode(this.mode)) {
          ...
          const graphemeIndex = currentIndex === -1 ? graphemes.length : currentIndex;
  ...
  private tryMoveCursorByState(delta: number): boolean {
    ...
    if (!Number.isInteger(state.cursorLine) || !Number.isInteger(state.cursorCol))
      return false;
    ...
    for (let step = 0; step < steps; step++) {
      ...
      if (direction === 'forward') {
        if (next >= len) {
          ...
          if (startType !== 'space') {
            while (next < len && this.charType(text[next], semanticClass) === startType)
              next++;
          }
          while (next < len && this.charType(text[next], semanticClass) === 'space')
            next++;
        } else {
          if (next < len - 1) next++;
          while (next < len && this.charType(text[next], semanticClass) === 'space')
          ...
          if (next >= len) {
            ...
            while (next < len - 1 && this.charType(text[next + 1], semanticClass) === t)
              next++;
          }
        }
      } else {
  ...
  private applyLineEndOperator(count: number, mode: 'change' | 'delete'): boolean {
    ...
    const targetEndAbs = this.getAbsoluteIndex(targetLine, targetLineText.length);

    const firstNonBlank = findFirstNonWhitespaceColumn(lines[cursor.line] ?? '');
  ...
  private moveCursorAfterDeleteToLineEnd(startLine: number, startCol: number): void {
    const lines = this.getLines();
  ...
  private deleteWithCharMotion(motion: CharMotion, targetChar: string): boolean {
    const line = this.getLines()[this.getCursor().line] ?? '';
    const col = this.getCursor().col;
    const count = this.takeTotalCount(1);
  ...
  private getModeRailItem(): { plain: string; styled: string } {
    const plain = this.getModeIcon();
    const modeColorize = this.getModeLabelColorizer();
...
export default function (pi: ExtensionAPI, options: PiVimOptions = {}) {
  ...
  let getSessionEntries: (() => readonly unknown[]) | null = null;
  const promptRail = new PromptRailStore();

  const stopPromptRail = pi.events.on(FANCY_FOOTER_WIDGET_CHANNEL, (raw) => {
    if (promptRail.apply(raw)) requestPromptRender?.();
  });
  const refreshSessionCost = () => {
    if (!getSessionEntries) return;
    if (promptRail.setTotalCost(collectSessionCost(getSessionEntries()))) {
      requestPromptRender?.();
    }
  };

  pi.on('session_start', (_event, ctx) => {
    getSessionEntries = () => ctx.sessionManager.getBranch();
    refreshSessionCost();
    ...
    const labelColorizers = t ? buildModeColorizers(t, modeColors) : null;
    // Both surfaces get their colorizers unconditionally. Host-synced modes
    // remain visually identical while still supporting the handoff override.
    ...
    ctx.ui.setEditorComponent((tui, theme, kb) => {
      ...
      promptExternalEditor = new PromptExternalEditor({
        agentDir: getAgentDir(),
        cwd: ctx.cwd,
        editor,
        mutedBorder: (text) => ctx.ui.theme.fg('dim', text),
  ...
  pi.on('message_end', refreshSessionCost);
  pi.on('session_compact', refreshSessionCost);

  pi.on('session_shutdown', (event) => {
    try {
      promptExternalEditor?.dispose();
