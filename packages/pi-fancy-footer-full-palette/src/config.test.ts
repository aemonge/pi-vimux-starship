...
test("footerConfigValidationErrors accepts a valid config", () => {
  assert.deepEqual(
    footerConfigValidationErrors({
      gaugeStyle: "bars",
      gaugeWidth: 8,
      providerStatus: { providers: ["openai-codex", "anthropic", "zai"] },
...
test("footerConfigValidationErrors accepts native Pi palette tokens", () => {
  assert.deepEqual(
    footerConfigValidationErrors({
      defaultTextColor: "mdHeading",
      extensionWidgets: {
        "galactica.palette": {
          iconColor: "customMessageLabel",
          textColor: "borderAccent",
        },
      },
    }),
    [],
...
test("footerConfigValidationErrors accepts compact quota percentages", () => {
  assert.deepEqual(
    footerConfigValidationErrors({ providerStatus: { display: "percent" } }),
    [],
  );
});
